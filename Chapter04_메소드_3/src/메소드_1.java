 ...
 *         ---------맨마지막에 사용하는 것이 아니라 중간에 사용이 가능하다
 *         메소드()
 *         {
 *         		if()
 *         		{
 ...
 *                                |
 *                        매개변수의 갯수나 데이터형이 다르다
 *                          aaa(int a)     //aaa(int b) 와는 같은 메소드(변수명이 다르다고 다른메소드X)
 *                          aaa(double d)
 *                          aaa(int a,int b) => 오버로딩(중복 정의 메소드)
 *          plus(int,int) plus(char,char) plus(double,double)    //셋 다 다른 메소드(데이터형이 다름)
 *          
 *   4. 메소드 호출
 *      --------
 *        = 인스턴스 메소드 => new를 이용해서 메모리 저장     //5장에서
...
public class 메소드_1 {
	...
	{
		String str=String.valueOf(num);    //문자열로 바꾼다 ==> String.valueOf()
		String res="";
	...
	{
		boolean bCheck=false;    //지역변수는 초기값 주기
		for(int i=0;i<arr.length-1;i++)
		{
			for(int j=i+1;j<arr.length;j++)
			{
...
//		int[] result1=sortASC(arr);
//		System.out.println("정렬후:");
//		System.out.println(Arrays.toString(result1));
//		int[] result2=sortDESC(arr);
...
//		System.out.println(Arrays.toString(result));
		...
		//reverse
		System.out.println(reverse(28347));
	}
