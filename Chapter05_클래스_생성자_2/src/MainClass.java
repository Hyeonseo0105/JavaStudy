 ...
 *                              A(String s,double d) => A(String,double)   | 오버로딩 안됨
 ...
 *                 공백이 있으면 안된다
 *                 특수문자 사용 => (_,$)
 *                 약속) 소문자로 시작한다 , 두번째 단어의 첫자는 대문자
 *                 *** 소프트웨어 이행
 *                     ----------- 한글에 문제가 생긴다
 *                                 가급적이면 영문사용 권장
 *                     윈도우 개발 => 서버 리눅스 사용
 *                     FullStack     AWS(PaaS)
 *                     => STS : Java11버전까지 지원(쓰고있는건 17)
 *                     => tomcat => 9버전 => 11
 *                        Spring-Boot => 2.7.4 =>3
 *                        javax =>
 *                     -----------------------------
...
//		try
//		{
//			ss=new ServerSocket(숫자);
//		}catch(Exception ex) {}
//	}
	class User
	{
		int[] arr=new int[5];
		//5개의 정수를 난수toString(null) => 정렬하는 프로그램 제작 => 출력
		User()
		{
