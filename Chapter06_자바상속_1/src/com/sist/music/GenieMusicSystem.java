...
public class GenieMusicSystem {
	...
	// => 생성자 , 초기화 블록 ∵배열에 값 못넣음
	{
		...
		try
		{
			Document doc=Jsoup.connect("https://www.genie.co.kr/chart/top200").get();//"사이트"의 HTML을 가져와라
			//HTML을 저장하는 공간 , HTML을 읽어서 데이터추출
			//데이터 저장 => HTML,JSON,XML => 데이터 추출 방식이 틀리다
			
			/*                  (속성)
			 *    HTML => tag/attrivute   <> </> <a/> => Markup lang
			 *            <a class="aaa"><\a>
			 *            --             ----
			 *    태그를 읽는 방법
			 *     Element => Tag
