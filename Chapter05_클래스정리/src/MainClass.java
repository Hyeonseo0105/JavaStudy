 ...
 *                       = 메모리 할당 (클래스 저장) => 호출된다
 *                       = 시작과 동시에 처리하는 명령문
 *                         (오라클 연결 , 네트워크 => 서버 구동, 자동로그인)
 *                       = 특징
 *                          => 클래스명과 동일
 *                             리턴형이 없다
 *                             여러개를 만들 수있다(오버로딩) => 중복 함수 정의
 *                             ** 오버로딩 조건
 *                                  1) 한개의 클래스에서 제작
 *                                  2) 메소드명이 동일
 ...
 *                                          ---------------클래스(3개이상)
 *                                          ---------------
 *                          => 멤버 메소드는 멤버변수 , static변수 , 지역변수를 사용할 수 있다
 *                          => 메소드 유형
 *                              리턴형     매개변수
 *                                ==> R(SELECT) => DML    //데이터 읽어올 때 사용
 *                               O         O
 *                               O         X
 *                              ---------------메소드 자체 처리(CRUD)
 *                                ==> CUD => (INSERT,UPDATE,DELETE)   //void 사용
 *                               X(void)   O
 *                               X(void)   X
 *                              ---------------리턴형이 많을 수 있다(웹)
 *                             사용법)
 ...
 *                      = 공유 메소드 => static
 *                          형식)                   기본형식
 *                             (static) [접근지정어] (static) 리턴형 메소드명(매개변수목록)
 *                             {
 *                                
 *                             }
 *                             => static메소드는 static변수 static메소드는
 *                                사용이 자유롭다
 *                             => static메소드 안에서는 인스턴스변수,메소드는
 *                                반드시 객체 생성후
 *                      = 상속을 방지 => final(종단메소드)
...
//	void aaa()
//	{
//		System.out.println(a);   //this.a this가 생략되어 있음
