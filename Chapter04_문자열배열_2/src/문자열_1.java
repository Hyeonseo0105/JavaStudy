 ...
 *                       boolean isLogint(String id,String pwd)
 *                 => String ss=s.substring(1);
 *                String substring(int begin, int end)
 *                
 *    => startsWith() : 시작문자열이 같은지 확인
 *       => 원형 : boolean starsWith(String msg)
 *                => if문에서 주로 처리
 *                String s="Hello Java"
 *                if(s.starsWith("H"))
 *              => 서제스트(자동완성기)
 ...
 *       => static : 자동 메모리 할당
 *          String.valueOf()
 *    => toString() : 생략이 가능 => 객체의 주소값(원시값) 출력\
 *    => indexOf() : 지정된 문자위치값 확인 (문자번호 0번부터)
 *       => 원형 : int indexOf(char)
 *                int indexOf(String s)
 *    => last IndexOf() : 지정된 문자위치값 확인 (문자번호, length()-1)
