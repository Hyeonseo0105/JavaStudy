...
public class Server implements Runnable{
	...
	class Client extends Thread
	{
		...
		public Client(Socket s)
		{
			...
			// PORT 연결선 => 서버는 고정 PORT / 클라이언트 자동 생성
			try
			{
				in=new BufferedReader(new InputStreamReader(s.getInputStream()));  //요청한 값을 클라이언트메모리에서 읽어옴
			...
			try
			{
				while(true)
				{
					...
					switch(protocol)
					{
						...
						{
							String data=st.nextToken();   // 웹 => jsp
