 ...
 *             *** 0으로 나눌 수 없다(0으로 나눌 경우 => 오류발생)
 *         % : 왼쪽 부호
 *             => 윤년 구하기, 숫자 자르기, 배수 구하기...
 *       = 비교연산자(==,!=,<,>,<=,>=) ===> 결과값은 true/false
 *         =>       문자,       정수, 실수
 *           'A'(문자) "A"(문자열) => 자바는 Unicode를 사용(모든 국가의 언어 사용이 가능)
 *                                      한글자당 바이트 2byte
 *           '임'(한글은 한글자당 2byte)
 *         == 같다
 *         != 같지않다
 ...
 *         || : 병렬연산자 (조건 2개 중 1개 이상이 true일 경우 true)
 *              (조건) || (조건)
 *              ----     ----
 *              주로 사용처 : 잘못된 입력 
 *       = 대입연산자(=,+=,-=)
