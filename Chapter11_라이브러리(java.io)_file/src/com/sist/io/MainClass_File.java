 ...
 *                     *(알아는두기)Set,Calendar
 *     ----------------------------------------------
 *      = java.io   => File,FileInputStream/FileOuputStream
 ...
 *                     ---------------------------- Git언어 동일
 *         보조스트림 : 읽는 속도 , 쓰는 속도를 빨리하기 위해 미리 메모리 임시 저장
 *                    String / StringBuffer
 *                  => Buffered~
 *                  => 전체를 메모리로 이동 => 메모리에 제어
 *         파일
 ...
 *                   => String은 저장이 불가능
 *                       => byte[]로 변경해서 저장
 *           파일 쓰기 : FileOuputStream / FileWriter
 *                   => String 자체를 저장
 *         네트워크
 *           => OutputStream(1byte)
...
public class MainClass_File {
	...
	public static void main(String[] args) {
		...
		{
			//directory 확인
			File dir=new File("c:\\javaDev");   //c:\javaDev는 오류 \두개써야
			// window => \   ,   max,linux => /
			// \는 반드시 \\를 사용한다
			// 가급적이면 절대경로를 이용한다
			// . => 현재폴더   ,   ..(cd(change directory) => 상위폴더
			// 폴더안에 있는 폴더/파일읽기
