 ...
 *     => 컴파일시에 예외처리 했는지 확인       => 확인이 없기 때문에 필요시에만 한다
 *     => 라이브러리중에 반드시 예외처리를 한다
 *     => java.io
 *         =>파일명이 틀린경우
 *         => FileNotFoundExcetion(경로명잘못씀)
 *         => IOException
 *     => java.net
 *         => URL주소,IP가 틀린경우
 ...
 *        ArrayIndexOfOutBoundsException : 배열의 인덱스범위를 벗어나는 경우
 *        
 *        NullPointerException : null인 클래스를 사용한 경우
 *                               ---- 주소가 없는 경우
 *        ArithmeticException : 0으로 나누는 경우
 *        
 *        ==> 에러 예상 : 문자열로 정수를 받아서 배열에 저장한 다음에 나누기를 한다
 *                     ---------        --------       ----
 *                NumberFormatException           ArithmeticException
 *                              ArrayIndexOfOutBoundsException
 *        ==> 사전에 에러 방지 => 예측
 *        
 *        ClassCastException : 형변환 (예)A a=new A();)
 */
import javax.swing.*;
class A
{
