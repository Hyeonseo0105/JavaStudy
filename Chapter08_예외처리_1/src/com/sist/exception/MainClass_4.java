 ...
 *      = 예외복구 => try~catch
 *        try => 정상 수행 문장 => 에러발생
 *        catch => try를 수행하는 과정 => 에러발생하면 1)확인 2)복구 => 처음부터 수행
 *                                             ---- getMessage()
 *                                                  printStackTrace() : 몇번째에 에러 있는지
 *        => catch를 여러번 사용이 가능
 *           catch => 순서가 존재한다(계층구조)
 *                    Throwable
 *                    ---------
...
public class MainClass_4 extends JFrame implements ActionListener {
	...
	public MainClass_4()
	{
		...
		ta=new JTextArea();
		JScrollPane js=new JScrollPane(ta);    //스크롤펜 = 스크롤바,오/왼 둘 다 생김
		b1=new JButton("Start");
		b2=new JButton("Exit");
		
		//윈도우 배치
		JPanel p=new JPanel();  //버튼을 묶어서 한줄에 배치
		p.add(tf);
		p.add(b1);
		p.add(b2);
